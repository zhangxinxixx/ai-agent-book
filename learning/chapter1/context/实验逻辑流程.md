...
\`trajectory.api_turns\`。这发生在后续上下文过滤之前，因此实验可以检查“提供商返回了什么”和
“下一轮实际发送了什么”。
...
| \`experiment_execution_accepted\` | \`analyze()\` | 每种模式是否都拿到了可以复查的模型响应 | 所有实验结论都适用于其他模型或任务 |
...
\`experiment_execution_accepted=true\`。这说明单组目录适合逐个检查和比较；如果要保存为正式结果，
还需要同一批次的一次完整五组运行。
