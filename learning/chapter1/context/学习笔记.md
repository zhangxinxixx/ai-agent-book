...
| `python -X utf8 run_experiment_1_1.py ...` | 运行有上下文契约和 SHA-256 的正式实验 | 是，多次 | `evidence.json` | 4 |
...
> 测试仍查找英文 `Vacuous by construction`，实现已改为中文“结构性必然”。这不是模型、网络或
...
初读可以跳过特定提供商的兼容字段、绘图样式和凭据检查细节，先吃透主循环与五种上下文契约。
...
| `no_tool_calls` | `tools_absent_every_turn`、`ToolActionCount`、`Outcome` | 请求中是否真的没有工具定义；模型会拒绝或给出无依据回答 | “没有工具调用”是结构性必然，不是模型能力比较 |
...
上述单分支命令是为了学习和观察，因而 `canonical_run=false`，不会改写教材引用的
...
[grounding.py](code/chapter1/context/grounding.py) 依据**实际发出的消息**计算，因此被隐藏了观测的实验组
